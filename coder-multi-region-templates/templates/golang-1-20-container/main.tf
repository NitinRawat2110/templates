module "aws_security_group" {
  source   = ".templates/modules/aws_security_group"
  region     = var.region
  environment = var.environment
}

module "aws_efs_service" {
  source   = ".templates/modules/aws_efs_service"
...
module "aws_ecs_task" {
  source   = ".templates/modules/aws_ecs_task"
...
module "aws_ecs_service" {
  ...
  source   = ".templates/modules/aws_ecs_service"
...
resource "coder_agent" "coder" {
  ...
  startup_script         = <<-EOT
    ...
    /tmp/code-server/bin/code-server --install-extension ms-python.flake8
    #/tmp/code-server/bin/code-server --install-extension ms-python.vscode-pylance
    ...
    #set PATH for go
    echo 'export PATH="/usr/lib/go-1.20/bin:$PATH"' >> ~/.bashrc
    
    # create necessary directories and files
    #mkdir -p config log work
    #touch config/testsettings.ini
    #if [ ! -d work/bin ]; then
