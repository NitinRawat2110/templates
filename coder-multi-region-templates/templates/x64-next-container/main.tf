...
resource "coder_agent" "coder" {
  ...
  startup_script         = <<-EOT
    ...
    # Setting the arch to amd64 as x86_64 is not a valid identifier for 64bit
    export ARCH=amd64
    export OS=linux

    # install code-server
    mkdir -p /tmp/code-server/lib/code-server-4.17.1-Linux-amd64
    ...
    # Setting the arch back to x86_64
    export ARCH=x86_64
    export OS=Linux
